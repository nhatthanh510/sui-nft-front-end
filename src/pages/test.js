...
const DEFAULT_URL = 'ipfs://bafkreibngqhl3gaa7daob4i2vccziay2jjlp435cf66vhono7nrvww53ty';
...
export default function Home() {
  ...
  useEffect(() => {
    if (connected && suiWallet) {
      suiWallet.getAccounts().then((accounts) => setAccount(accounts[0]), setMsgNotice);
    } else {
      setAccount(null);
    }
  }, [connected, suiWallet]);
  ...
  const onCreateClick = useCallback(async () => {
    ...
    try {
      ...
      setMsgNotice(`NFT successfully created. ${nftID ? `ID: ${nftID}` : ''}`);
  ...
  return (
    <div className={styles.container}>
      ...
      <main className={styles.main}>
            ...
            {connected ? (
              <>
                ...
                <div>
                  <h2>Create NFTs</h2>
                  <label>
                    Name: <input type="text" placeholder={DEFAULT_NAME} ref={nameRef} />
                  </label>
                  <label>
                    Description:{' '}
                    <input type="text" placeholder={DEFAULT_DESCRIPTION} ref={descRef} />
                  </label>
                  <label>
                    Image url: <input type="text" placeholder={DEFAULT_URL} ref={urlRef} />
                  </label>
              ...
              <button type="button" onClick={onConnectClick} disabled={connecting}>
                Connect
              </button>
            )}
          </div>
        ...
        {msgNotice ? (
          <div className="error">
            <pre>{msgNotice.message || JSON.stringify(msgNotice, null, 4)}</pre>
          </div>
        ) : null}
      </main>
    </div>
